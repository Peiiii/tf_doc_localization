...
class Detector:
    def __init__(self):
        self.graph=self._load_graph(frozen_graph_filename=frozen_graph_filename)
        self.sess = tf.Session(graph=self.graph)
    def predict_from_file(self,f):
        img=Image.open(f).convert('RGB').resize((600,800))
        img=np.array(img)
        y=self.predict(img)
        return y
    def show(self,img):
    ...
    def predict(self,img):
        ...
        img_nd=img
        inputs = self.graph.get_tensor_by_name('input:0')
        activation_map = self.graph.get_tensor_by_name("heats_map_regression/pred_keypoints/BiasAdd:0")
...
def demo():
    ...
    for img_name in all_imgs:
        img_nd = np.array(Image.open(img_name).convert('RGB').resize((600, 800)))
